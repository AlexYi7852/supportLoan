...
Page({
  ...
  data: {
    ...
    btnTxt: '开始',
    random_number: '7896',
    token_random_number: '2jkgjgfyuduyfghjkdytfugihjok',
    viewSrc: '',
    countDown: '',
    userInfo: '',
    face_type: '1', // 人脸识别来的类型 1、修改密码 2、修改手机号 3、贷款    4、 实名认证验证手机号  5 贷款申请修改信息  6 商贷识别
    time: 5,
    timeShow: false,
    clock: null,
  ...
  rawValidateVideo(path) {
    ...
    app.api
      .rawValidateVideo(path, {
        tokenRandomNumber: this.data.token_random_number
      })
      .then(res => {
        console.log('======================video==========================')
        console.log(res)
        if (res.code == 0) {
          this.videoRawVerify(res.data.token_video)
        } else {
          wx.showModal({
            title: '错误信息',
            content: res.message,
    ...
    wx.request({
      ...
      success: function(res) {
        if (res.data.code == 0) {
          ...
          console.log(res)
          console.log('erroroiuououioyiuyh')
          wx.showModal({
            title: '错误信息',
  ...
  faceSuccessLink() {
    ...
    switch (+face_type) {
      case 1: //修改密码
        wx.redirectTo({ url: '../forget_password/forget_password_1/index' })
        break
      case 2: //修改手机号
        wx.redirectTo({
      ...
      case 3: //贷款申请流程
        wx.redirectTo({
          url: '../../apply_loan/apply/step6/index?show_type=3'
        })
        break
      case 4: //实名认证验证绑定手机号
        wx.redirectTo({
      ...
      case 5: //修改贷款申请
        wx.redirectTo({
          url: '../../apply_loan/apply_edit/step6/index?show_type=3'
        })
        break
      case 6: //商业贷款
        wx.redirectTo({
          url: '../../index/commercial_loan_apply/step3?show_type=3'
        })
        break
      default:
        break
