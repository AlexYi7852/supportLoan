...
Page({
  ...
  data: {
    ...
    year: yearArr, //[2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018],
    yearIndex: 5, //入学年份
  ...
  renderingSelect() {
    ...
    if (this.data.applyData.enrollmentYear == currentYear) {
      ...
      this.setData({
        educationData: educationData2 //如果不是2018年没有 预科
      })
    }
    if (education) {
      ...
      if (educationIndex > 0) {
        this.setData({ educationIndex: educationIndex })
      }
    }
  ...
  userDetail() {
    ...
    app.api
      ...
      .then(res => {
        ...
        if (res.code === 0) {
          let cityCodes = {
            proCode: res.data.collegeAddrProvId,
            cityCode: res.data.collegeAddrCityId,
            countyCode: res.data.collegeAddrAreaId
          }
          this.setData({
            cityCodes: cityCodes,
            applyData: res.data,
